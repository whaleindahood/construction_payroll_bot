        ...
        try:
            with self.sessions() as session, session.begin():
                ...
                if start_date is not None:
                    ...
                    if first_shift and start_date > first_shift:
                        raise DomainError(
                            "Дата начала не может быть позже существующих смен."
                        )
                before = {
                    "name": employee.name,
        ...
        *,
        ...
        with self.sessions() as session:
            ...
            for employee_id in ids:
                ...
                self._rate(session, employee_id, work_date, object_id)
        ...
        try:
            with self.sessions() as session, session.begin():
                ...
                for employee_id in ids:
                    ...
                    if employee is None or employee.status != "active":
                        raise NotFound("Активный сотрудник не найден.")
                    rate = self._rate(session, employee_id, work_date, object_id)
                    row = Attendance(
                        employee_id=employee_id,
                        object_id=object_id,
                        work_date=work_date,
                        rate_snapshot=rate,
                        earned_amount=as_money(rate) if rate is not None else None,
    ...
    def summary(
        ...
        *,
        ...
        with self.sessions() as session:
            ...
            earned, unrated = session.execute(
                select(
        ...
        *,
        ...
        try:
            with self.sessions() as session, session.begin():
                ...
                if employee is None:
                    raise NotFound("Сотрудник не найден.")
                if session.get(WorkObject, object_id) is None:
                    raise NotFound("Объект не найден.")
                row = Payment(
                    employee_id=employee_id,
                    object_id=object_id,
